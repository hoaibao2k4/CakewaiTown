...
const signUpForm: React.FC = () => {
  ...
  const validatePassword = useCallback((value: string) => {
    ...
    else if (value.length >= 9 && value.length < 12) setPasswordStrength('fair');
  ...
  const validateName = (nameValue: string): string => {
    if (/^\d+$/.test(nameValue)) return 'Bạn có chắc nhập đúng tên của mình không';
    if (nameValue.length < 3) return 'Tên cần ít nhất 3 ký tự chữ';
    if (/[^\p{L}\d\s]/u.test(nameValue)) return 'Tên không được chứa ký tự đặc biệt';
    return '';
  };
  ...
  const handleRegister = useCallback(
    (e: React.FormEvent) => {
      ...
      const passwordMismatchError = password !== confirmPassword ? 'Mật khẩu không trùng khớp' : '';

      setNameError(nameValidationError);
      setEmailError(emailValidationError);
      setConfirmError(passwordMismatchError);

      if (!nameValidationError && !emailValidationError && !passwordMismatchError) {
  ...
  return (
    ...
    <div
style={{ backgroundImage: `url(${"/images/images_authen/authImage.png"})` }}
...
>
    ...
    <div className=" flex flex-col justify-center items-center   min-h-[calc(100vh-15rem)] md:min-h-[calc(100vh-25rem)] lg:min-h-[calc(100vh-36rem)] py-2 md:py-4 lg:py-8  lg:justify-start  ">
      ...
      Tạo tài khoản của bạn ngay. Miễn phí và chỉ mất một phút!
          ...
          <form onSubmit={handleRegister} className="flex flex-col items-center w-full px-4">
            <div className="relative my-3">
                ...
                />
                <label htmlFor="name" className="absolute start-1 top-2 z-0 origin-[0] -translate-y-4 scale-75 transform bg-gray-100 px-2 text-sm text-gray-500 duration-300 peer-placeholder-shown:top-1/2 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:scale-100 peer-focus:top-2 peer-focus:-translate-y-4 peer-focus:scale-75 peer-focus:px-2 peer-focus:text-blue-600 dark:text-gray-400 peer-focus:dark:text-blue-500 rtl:peer-focus:left-auto rtl:peer-focus:translate-x-1/4">
                Tên của bạn
                </label>                

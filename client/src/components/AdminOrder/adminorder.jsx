...
function AdminOrder() {
    ...
    const [isModalOpen, setIsModalOpen] = useState(false); // Thêm trạng thái để điều khiển modal
    ...
    const handleOrderStatus = async (id, currentStatus) => {
        try {
            const newStatus = currentStatus === "Được duyệt" ? "Đang xử lý" : "Được duyệt";
    ...
    return (
        <div className="p-4">
            <div className='p-3 flex justify-between items-end'>
                ...
                <Button type="primary" onClick={handleAdd} className="mb-4" style={{ backgroundColor: "#664545" }}>Thêm Đơn Hàng</Button>
                <AddOrderModal
                    ...
                    <tbody>
                        {orders?.map((order, index) => (
                            <tr key={order.id} className="hover:bg-gray-100 transition-all text-gray-800">
                                ...
                                <td className="border-y border-gray-200 py-3 px-4 text-center">{order.order_status === 'Được duyệt' ? 'Được duyệt' : 'Đang xử lý'}
                                    <button className="bg-green-500  text-white border border-[rgb(102,69,69)] rounded px-3 py-1 hover:opacity-90 transition"
